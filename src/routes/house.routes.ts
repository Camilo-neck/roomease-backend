...
router.get(
	"/:houseId",
	[Auth, ParamsValidator(paramsSchema), HouseExist(FIELD_TYPES.PARAMS), BelongsToHouse],
...
router.delete(
	"/:houseId",
	[Auth, ParamsValidator(paramsSchema), HouseExist(FIELD_TYPES.PARAMS), Owner],
	asyncError(houseController.delete),
);
router.put(
	"/join/:house_code",
	[Auth, ParamsValidator(paramsHouseCode), HouseExist(FIELD_TYPES.PARAMS)],
